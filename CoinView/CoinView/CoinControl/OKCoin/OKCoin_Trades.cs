...
{
    public partial class OKCoin_Trades : UserControl, ICoinControl
    {
        public OKCoin_Trades()
        {
            InitializeComponent();
        ...
        #region 获取数据&刷新显示
        public const string url = "https://www.okcoin.com/api/trades.do?symbol=[CURR_A]_[CURR_B]&since=0"; // since<=0 为返回最新,参数since对应json对象中的tid
        private int currentCoinIndex;
        private JArray ja_trades; // 暂时存放jarray数据,时间顺序(date值, 更严格地说是tid的值)按照从小到大(也即从古到今)
        ...
        private void loadData()
        {
            ...
            try
            {
                using (Stream stream = hwr.GetResponse().GetResponseStream())
                {
                    ...
                    ja_trades = JArray.Parse(rtnJson);
                    sr.Close();
                }
                errMsg = "";
            }
